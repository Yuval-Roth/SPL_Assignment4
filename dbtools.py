...
class Dao(object):
    ...
    def update(self, dto_instance, **keyvals):
        upd_dict = vars(dto_instance)
        column_names = upd_dict.keys()
        params = list(upd_dict.values()) + list(keyvals.values())
        set_clause = ','.join([col + '=?' for col in column_names])
        where_clause = ' AND '.join([col + '=?' for col in keyvals.keys()])
        
        stmt = 'UPDATE {} SET {} WHERE {}' \
               .format(self._table_name, set_clause, where_clause)
